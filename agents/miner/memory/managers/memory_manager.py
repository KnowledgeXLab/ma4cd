...
class UnifiedMemoryManager:
    ...
    # [新增] 兼容 EvolutionEngine 的 storage 属性
    @property
    def storage(self):
        """
        提供给 miner_evolution_engine 使用的 storage 接口别名
        指向 persistent_memory (长期存储)
        """
        return self.persistent_memory
    
    # [新增] 兼容 EvolutionEngine 的 get_best_experience 方法
    def get_best_experience(self, domain: str) -> Optional[Dict]:
        """
        获取某个域名的历史最佳经验（配置）
        供 miner_evolution_engine.py 调用
        """
        try:
            # 优先从持久化记忆中获取
            if self.persistent_memory:
                # 尝试调用底层存储的 get_website_knowledge
                if hasattr(self.persistent_memory, 'get_website_knowledge'):
                    knowledge = self.persistent_memory.get_website_knowledge(domain)
                    # 如果返回了数据，且包含 config 或 strategy，则返回
                    if knowledge:
                        # 构造符合 EvolutionEngine 期望的格式
                        return {
    ...
    def get_recommendation(self, domain: str, site_profile: Dict) -> Dict[str, Any]:
        ...
        try:
                ...
                try:
                    ...
                    if hasattr(self.persistent_memory, 'get_website_knowledge'):
                        ...
                        if historical:
                            ...
                            recommendation['reasoning'].append(f"找到完全匹配的历史记录")
                    else:
                        # 查找相似网站
                        if hasattr(self.persistent_memory, 'find_similar_websites'):
                            similar_sites = self.persistent_memory.find_similar_websites(site_profile, limit=3)
                            if similar_sites:
                                recommendation['similar_sites'].extend(similar_sites)
                                recommendation['confidence'] += 0.2
                                recommendation['reasoning'].append(f"从长期记忆找到 {len(similar_sites)} 个相似网站")
                except Exception as e:
            ...
            if self.chroma_memory:
                try:
                    if hasattr(self.chroma_memory, 'find_similar_websites'):
                        ...
                        if similar_sites:
                            ...
                            # 基于相似网站调整策略
                            if similar_sites[0].get('similarity', 0) > 0.7:
