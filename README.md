...
- `Scout`：执行广域侦察，锁定候选站点
- `Miner`：对候选站点做深度挖掘，识别 L1/L2/L3/L4 资产
...
└── batch_commander.py     # 批量战役入口
```

## 核心流程

推荐从 [main_workflow.py](main_workflow.py) 理解项目：
...
export MA4CD_LLM_BASE_URL="https://your-openai-compatible-endpoint/v1"
```
...
```bash
python batch_commander.py
```

该脚本会按照预设国家和领域批量执行任务，并带有断点续跑与冷却逻辑。

### 7. 航天 26 任务一键批跑

```bash
# 建议先做稳态限流（可选）
export MA4CD_MINER_CONCURRENCY=2
export MA4CD_INSPECTOR_MAX_WORKERS=4
export MA4CD_INSPECTOR_BATCH_SIZE=4
export MA4CD_INSPECTOR_LLM_RETRIES=5
export MA4CD_REPORT_TIMEOUT=240

python batch_run_26_aerospace.py
```

执行后会在 `reports/batch26_<timestamp>/` 生成三份汇总：

- `summary.json`：全量结构化结果
- `summary.csv`：可直接用 Excel 打开
- `summary.md`：可读性汇总报告

同时会保存 `checkpoint.json`，中断后再次执行同一脚本可继续跑未完成任务。
